...
export async function readPdf(bytes) {
  ...
  const pages = [];
  const items = [];
  // The printed page box, per page, in PostScript points — the only place the
  // size a card declared can be read back off the thing that goes to a printer.
  const pageSizes = [];
  ...
  return { pageCount: doc.numPages, pageSizes, pages, text: pages.join(''), items };
}
