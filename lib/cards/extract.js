...
/**
 * The page geometries a card can be printed at, keyed by the `cardSize` front
 * matter an article may set. `letter` is the default and what every crew and
 * position card uses. `index` is a 4x6in index card printed landscape — for a
 * card short enough to live in a shirt pocket rather than a bag.
 *
 * `stylesheet` names the extra sheet in this directory that redefines the page
 * box and the three knobs for that size; the base sheet is always used.
 * `width` and `height` are the printed page in PostScript points, which is
 * what a rendered PDF reports back and what the output gate compares against.
 */
export const CARD_SIZES = {
  letter: { label: 'Letter, 8.5 by 11in', width: 612, height: 792 },
  index: { label: 'index card, 6 by 4in', width: 432, height: 288, stylesheet: 'card-index.css' },
};

export const DEFAULT_CARD_SIZE = 'letter';

/**
 * The size an article declares, validated here rather than in the renderer: a
 * typo in `cardSize` would otherwise reach the browser as a missing stylesheet
 * and come back as a Letter card nobody asked for.
 */
export function cardSizeOf(data) {
  const size = data?.cardSize ?? DEFAULT_CARD_SIZE;
  if (!Object.hasOwn(CARD_SIZES, size)) {
    throw new Error(
      `unknown cardSize ${JSON.stringify(size)} — known sizes are ${Object.keys(CARD_SIZES).join(', ')}`,
    );
  }
  return size;
}
...
/**
 * `Printable` means "there is something here to print", which is not the same
 * as "the build renders this". The 2022 slide deck and the NVYFL rules pages
 * are all tagged `Printable` and all link documents somebody else produced.
 *
 * So an article declares itself a generated card by linking its own card:
 * `/cards/<slug>.pdf`. That is a fact already in the markdown — it is how a
 * reader gets the card at all — rather than a list in this file that a CMS
 * editor cannot reach. Tagging a new page `Printable` can then never fail a
 * build it had no way to know about.
 */
export const declaresCard = (article) =>
  article.body.includes(`(${cardUrl(article.slug)})`) ||
  article.body.includes(`"${cardUrl(article.slug)}"`);
...
// The full HTML entity table, not a hand-written handful. A raw HTML block in
// an article may use any named entity, and one this misses reaches the
// completeness gate as the literal letters "ndash" — which the PDF will not
// contain, so the gate reports content dropped that was never dropped.
const tidy = (s) => decodeHTML(s).replace(/\s+/g, ' ').trim();
...
/**
 * A block broken into the smallest pieces the card still has to say: one per
 * paragraph, per list item, per table cell, per figure caption.
 *
 * The completeness gate compares these against the text read back out of the
 * PDF, and it compares them one at a time on purpose. A whole table matched as
 * one string fails the moment Chromium repeats a header row across a column
 * break; a whole list fails on the "2." it draws between the items. Neither is
 * a dropped bullet, and a gate that cries wolf about them is a gate someone
 * turns off.
 */
function blockChunks(group) {
  if (group[0]?.type === 'html_block') {
    return group
      .map((t) => t.content ?? '')
      .join('')
      .replace(/<[^>]*>/g, '\n')
 ...
 *
 * Returns `{ slug, title, size, subtitle, sections, provenance }`, where each section
  ...
  return {
    ...
    sections: sections
      ...
      .map((s) => ({
        ...
        // What the card has to say, in the pieces the gate checks it in — a
        // dropped last bullet is invisible in a whole-section comparison, and
        // is exactly what overflow looks like.
        chunks: [s.title, ...s.blocks.flatMap(blockChunks)].filter(Boolean),
