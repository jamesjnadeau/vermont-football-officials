...
import { CARD_STYLESHEET_PATHS, renderCardHtml } from './card-template.js';
import { CARD_SIZES, cardImages, cardModel, printableArticles } from './extract.js';
...
export function cardSources() {
  return {
    ...Object.fromEntries(
      Object.entries(CARD_STYLESHEET_PATHS).map(([name, file]) => [name, readFileSync(file, 'utf8')]),
    ),
...
export async function openRenderer() {
  ...
  return {
    /** HTML in, a two-sided PDF out, at whatever page box the card declares. */
    async pdf(html, { size = 'letter' } = {}) {
      const p = await ready();
      await p.setContent(html, { waitUntil: 'load' });
      await p.evaluate(() => document.fonts.ready);
      // A data URI the browser cannot decode leaves blank space, not an error.
      // The output gate catches it in the finished PDF; catching it here says
      // which figure, before two pages of layout are built around a hole.
      const broken = await p.evaluate(() =>
        [...document.images].filter((img) => !img.complete || img.naturalWidth === 0).length,
      );
      if (broken > 0) throw new Error(`${broken} figure(s) failed to decode`);
      // `preferCSSPageSize` is what makes the size sheets work at all: the
      // page box comes from the stylesheet's `@page`, not from here. The width
      // and height are those same numbers, passed as the fallback for a sheet
      // that somehow named no size — in inches at 72 to the inch, because
      // Playwright takes px/in/cm/mm and the table is kept in points, the unit
      // a rendered PDF reports back.
...
export async function renderCard(
  ...
  const pdf = await renderer.pdf(html, { size: model.size });
...
/**
 * Renders every printable article into `outputDir`, reusing one browser and
 * the cache. Called from the Eleventy build; returns what it did, so the
 * caller can say so.
 */
export async function writeCards({
  inputDir = 'content/information',
  outputDir = '_site/cards',
  staticDir = DEFAULT_STATIC_DIR,
  cacheDir = '.cache/cards',
  log = console.log,
} = {}) {
  const articles = printableArticles(inputDir);
  ...
  try {
    for (const article of articles) {
      ...
      const { pdf, cached } = await renderCard(model, {
        article,
        cacheDir,
        staticDir,
        renderer,
      ...
      const file = path.join(outputDir, `${model.slug}.pdf`);
      writeFileSync(file, pdf);
      written.push({ slug: model.slug, file, cached });
    }
  } finally {
    await renderer.close();
  }

  const rendered = written.filter((w) => !w.cached).length;
  log(`cards: ${written.length} written, ${rendered} rendered, ${written.length - rendered} cached`);
  return written;
}
