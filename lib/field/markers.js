...
/**
 * A mark must be renderable at the origin, so the drawing page can move it
 * many times a second by updating a wrapper `<g transform="translate(...)">`
 * instead of re-serialising it every frame.
 *
...
function place1(p, view) {
  return [x(p.across), yardToY(view, p.down)];
}

function movementCoords(points, view) {
  return points.map((p) => place1(p, view));
}
...
/**
 * A movement's `d`, allowing a point to arrive on a curve.
 *
 * A point may carry `via`: one control point makes the segment into it a
 * quadratic, two make it a cubic. Straight segments stay `L`, so a path with
 * no `via` anywhere emits exactly what `coordsToD` would. Curves exist
 * because a few diagrams show a runner's arc rather than a straight release,
 * and an arc drawn as three straight segments reads as three decisions.
 */
function pointsToD(points, view) {
  const parts = [];
  points.forEach((p, i) => {
    const [px, py] = place1(p, view);
    if (i === 0) {
      parts.push(`M ${num(px)} ${num(py)}`);
      return;
    }
    const via = p.via ?? [];
    const ctrl = via.map((c) => place1(c, view));
    if (ctrl.length === 0) parts.push(`L ${num(px)} ${num(py)}`);
    else parts.push(
      `${ctrl.length === 1 ? 'Q' : 'C'} ` +
        ctrl.map(([cx, cy]) => `${num(cx)} ${num(cy)}`).join(' ') +
        ` ${num(px)} ${num(py)}`,
    );
  });
  return parts.join(' ');
...
/**
 * A movement path: a dotted line, straight `L` segments between however many
 * `points` are given — a straight arrow is just the two-point case, a bent
 * one needs no function of its own, and a point carrying `via` arrives on a
 * curve. `label`, if given, is a `note`-styled caption sitting above the
 * path's last point.
 *
 * `arrow` defaults to true because a movement almost always has a direction.
 * The exception is a path that means "this whole span is yours" rather than
 * "go here" — a coverage sweep across the back of the formation — where an
 * arrowhead would assert a direction the mechanic doesn't have.
 */
export function movement({ points, label, arrow = true }, view) {
  const d = pointsToD(points, view);
  const head = arrow ? ' marker-end="url(#ar)"' : '';
  let svg = `<path d="${d}" class="mv"${head}/>`;
  if (label != null) {
    const [lastX, lastY] = place1(points[points.length - 1], view);
 ...
 *
 * `font-size` is the one exception, and always emitted. Omitting an
 * attribute is only safe when what SVG falls back to *is* the stated
 * default, which holds for every other one here — the initial `fill` is
 * black, the initial weight is normal, there is no decoration and no
 * transform. It does not hold for size: with no `font-size` anywhere (a
 * label carries no class, so nothing in `STYLE` reaches it) text renders at
 * the SVG default of 16, and a caption nobody has resized would come out a
 * third bigger than the 12 this function says it is. Writing it costs a
 * handful of bytes; leaving it out made the default a lie.
 */
export function label(
  ...
  const cxs = num(cx);
  const cys = num(cy);
  const attrs = [`x="${cxs}"`, `y="${cys}"`, `font-size="${num(size)}"`];
