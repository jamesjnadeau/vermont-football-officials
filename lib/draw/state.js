 ...
 * draws "HL" is the kind of small disagreement that costs somebody a
 ...
 * an entry here, which is what makes a decoded share link safe by construction
 * rather than by remembering to check it at the call site.
 */
...
/**
 * Marks this board once used, and what they are called now.
 *
 * The 2026 NFHS Game Officials Manual renamed the Head Linesman to Head Line
 * Judge (Definition of Terms, p. 9) and its MechaniGrams letter him HL. A
 * share link written before that carries `LM`, and one an official has
 * already texted to a crewmate has to keep opening the same board — so the
 * old spelling is translated on the way in rather than left in `OFFICIALS`,
 * where a palette button and a diagram would both start drawing it again.
 */
const RENAMED_MARKS = { LM: 'HL' };
...
// these values ride in a share link and end up as attributes on a `<text>`
// in a page on this association's domain. The escaping in
...
/**
 * A yard of slack on every edge of the bounds below.
 *
 * Two reasons, and both are about the boundary rather than about the field.
 * `clampToFrame` clamps a token's *centre* to the frame, so a token dragged
 * hard into a corner sits exactly on the edge and must still be legal. And
 * a share link rounds coordinates to a tenth of a yard, so that same token
 * can come back half a rounding step *outside* the edge it was clamped to —
 * without slack, a board would stop round-tripping through its own link.
 */
const COORD_MARGIN_YARDS = 1;

/**
 * The furthest a coordinate can be and still be somewhere a view could show
 * it: the union of every view's frame, plus the margin above.
 *
 * The union, and never the current view's own frame. **The view is a
 * camera**: changing it moves nothing, so a bound that depended on it would
 * make a token legal under one crop and illegal under the next, and
 * `setView` would stop being the one-field change this whole module is
 * built around.
 *
 * This is what stops a crafted link (`codec.js`) planting a token nobody
 * can see. A finite coordinate is not enough — `across: 199.9` is a
 * perfectly good number and lands a token miles off the side of a 270-unit
 * viewBox, where it is never visible and never clickable and yet still
 * carries a `tabindex` and an accessible name. A keyboard or screen-reader
 * user then tabs through phantoms, which is the whole defect the tokens'
 * `tabindex` exists to avoid in the first place.
 *
 * Derived from `views` rather than written down, so retuning a view's
 * `scaleY` or `height` retunes this with it. That does mean narrowing a
 * view could, in principle, put an old link's token outside the bounds —
 * but a token only that view could ever show was going to become invisible
 * on that change anyway, so there is nothing here to preserve.
 *
 * Note what is deliberately *not* bounded: the sidelines. The wings work
 * the sideline from out of bounds, and the committed diagrams put them at
 * ±30.67 yards across — outside the touchlines at ±26.67. The frame is 36
 * yards wide either side of centre precisely to hold them, and a bound that
 * reached in far enough to move an official would be teaching the wrong
 * mechanics to enforce a rule nobody asked for.
 */
export const COORD_BOUNDS = (() => {
  const downs = Object.values(views).flatMap((view) => [yToYards(view, 0), yToYards(view, view.height)]);
  return {
    across: [xToYards(0) - COORD_MARGIN_YARDS, xToYards(VIEWBOX_WIDTH) + COORD_MARGIN_YARDS],
    down: [Math.min(...downs) - COORD_MARGIN_YARDS, Math.max(...downs) + COORD_MARGIN_YARDS],
  };
})();

/**
 * A limit and not a clamp, deliberately. `clampToFrame` folds a value back
 * because a drag that overshoots plainly meant the edge; a coordinate from
 * outside every frame there is meant nothing, and moving it to the edge
 * would put a token nobody placed where it can be mistaken for one somebody
 * did.
 */
function requireCoordinate(value, what, [low, high]) {
  requireNumber(value, what);
  if (value < low || value > high) {
    throw new Error(`${what} must be between ${low} and ${high} yards, got ${value}`);
  }
  return value;
}
...
function requirePoint(point, what) {
  ...
  return {
    across: requireCoordinate(point.across, `${what}.across`, COORD_BOUNDS.across),
    down: requireCoordinate(point.down, `${what}.down`, COORD_BOUNDS.down),
  };
}
...
export function addToken(state, { type, mark, kind, across, down, ...style }) {
  ...
  let token;
  if (type === 'official') {
    const current = RENAMED_MARKS[mark] ?? mark;
    if (!OFFICIALS.some((o) => o.mark === current)) {
      throw new Error(
        `unknown official mark "${mark}" — must be one of: ${OFFICIALS.map((o) => o.mark).join(', ')}`,
      );
    }
    token = { id: nextId(state.tokens, 't'), type, mark: current, ...at };
  } else if (type === 'player') {
