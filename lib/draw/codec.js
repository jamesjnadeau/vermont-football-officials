 ...
 * all after the `#`, and from Task 6 on that payload carries free text, a
 * colour, a size and an angle that all end up as content and attributes in
 * the document. So this file has exactly one security rule, and it is a
 * rule about where the checking happens rather than what it checks:
 *
...
/**
 * A caption's six styling fields: the wire key, the state field, and the
...
function encodeToken(token) {
  ...
  // board that costs real length in a link — its words plus six styling
