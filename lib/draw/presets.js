 ...
 *     from `.../scratchpad/extract/positions.json` — the output of the
 *     Task 4 Step 1 verification gate, which parsed them back out of the
 *     committed position cards and crew cards (`static/images/
 *     position-cards/*`, `static/images/kicking-plays/`) rather than having
 *     them typed in from memory. A crew-of-4 preset is deliberately absent:
 *     the gate found the real crew-of-4 art does not agree with "crew-of-5
 *     minus the Back Judge" (see the plan's Task 4 Step 1 correction), so
 *     shipping that derivation would teach four officials to stand in the
 *     wrong places. If crew-of-4 presets are wanted later they need their
 *     own extraction from that art, not a formula applied to this data.
 *
...
/**
 * Punt and Spot are named in the plan alongside these three, but nothing in
 * this repo holds verified officials' positions for them: the Task 4 Step 1
 * gate was scoped to `kickoff`, `field-goal.svg` and `goal-line.svg` only,
 * and no punt or spot position cards were parsed. Shipping a punt or spot
 * preset without officials would fail "the second group is the one carrying
 * officials"; shipping one with guessed positions would fail the whole
 * point of Step 1. So both are left out — see `docs/sources.md` for the gap
 * this records, per the plan's own instruction not to guess one.
 */
export const SITUATIONS = [KICKOFF, FIELD_GOAL, GOAL_LINE];
...
const WING_T = {
  ...
  tokens: [
    lineman(0), // C
    lineman(-0.67), // LG — 2' split from the centre, per source
    lineman(0.67), // RG
    lineman(-1.33), // LT — 2' split from the guard
    lineman(1.33), // RT
    lineman(2.0), // TE, strong side — 2' split from the tackle
    lineman(-9.33), // SE, weak side — generic wide split, no TE that side
    back(0, -1), // QB, under center
    back(0, -4), // FB, "heels four yards behind the football"
    back(3.0, -1), // Wingback — "1 yard deep, 1 yard outside the TE"
    back(-1.33, -4), // HB, weak side — FB's depth, outside foot of the tackle
  ],
};
...
const POWER_I = {
  ...
  tokens: [
    ...
    lineman(-2.5), // LT
    lineman(2.5), // RT
    lineman(-3.7), // TE, weak side — no WR in this formation
    lineman(3.7), // TE, strong side
    back(0, -1), // QB, under center
    back(0, -4), // FB, ~3 yd behind the QB
    back(0, -6), // RB (tailback), ~2 yd behind the FB, stacked
    back(-3, -4), // H-back, FB's depth, offset to a side
  ],
};
...
const SHOTGUN = {
  ...
  tokens: [
    ...
    back(-2, -5), // RB, alongside the QB
    back(-12, 0), // X, split end, on the line
    back(-6, -1), // H, inside slot, a step off the line
    back(6, -1), // Y, inside slot, a step off the line
    back(12, -1), // Z, flanker, off the line
  ],
};
...
const TRIPS = {
  ...
  tokens: [
    ...
    lineman(2.5), // RT
    back(0, -5), // QB, shotgun depth
    back(-2, -5), // RB, backside
    back(-12, 0), // Z, single receiver, isolated backside
    back(6, 0), // #3, innermost trips receiver, attached
    back(12, -1), // #2, trips slot, a step off the line
    back(18, 0), // #1, trips outside receiver, on the line
  ],
};
