 ...
 *
 * Presets, arrows, captions and the share link are later tasks; the layers
 * they fill (`board-arrows`, `board-text`) are rendered empty here.
 */
...
function buildToolbar() {
  ...
  const button = (label, name, attribute) =>
    `<button type="button" class="btn btn-outline-dark btn-sm" ${attribute}` +
    ` aria-label="Add ${escapeText(name)}">${escapeText(label)}</button>`;

  toolbar.innerHTML =
    `<div class="btn-group" role="group" aria-label="Officials">` +
    OFFICIALS.map((o) => button(o.mark, o.name, `data-add-official="${o.mark}"`)).join('') +
    `</div>` +
    `<div class="btn-group" role="group" aria-label="Players">` +
    PLAYERS.map((p) => button(p.label, p.name, `data-add-player="${p.kind}"`)).join('') +
    `</div>` +
  ...
  hint.textContent =
    'Add a marker, then drag it into place. Or tab to one and nudge it with the arrow keys — ' +
    'hold Shift for a full yard — and remove it with Delete. Ctrl/Cmd-Z undoes.';
...
function mount() {
  ...
  const { viewBox, markup } = renderBoard(board.view);
