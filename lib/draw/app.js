 ...
 *
 * The share link is a later task.
 */
import { renderBoard, layerId } from './board.js';
import { official, player, movement, label } from '../field/markers.js';
...
function syncProperties() {
  ...
  for (const control of props.querySelectorAll('[data-field]')) {
    ...
    setValue(control, field === 'color' ? asHex(token.color) : String(token[field]));
  }
...
/** Enter finishes a caption, the way it finishes a bent arrow. */
function onPropsKeyDown(event) {
  if (event.key !== 'Enter' || event.target.dataset.field !== 'text') return;
  event.preventDefault();
  event.target.blur();
}
