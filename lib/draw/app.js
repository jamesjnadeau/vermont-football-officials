...
import { decode, encode, tooBigForALink } from './codec.js';
...
/**
 * The board lives after the `#`, in `?`-style pairs so a second key could be
 * added beside it without moving this one or breaking a link already sent.
 *
...
function select(id) {
  ...
  syncProperties();
  // Selecting without changing the board never reaches `render()`, and the
  // Delete button says what it will remove — so it has to be told here too.
  updateDeleteButton();
}
...
/**
 * The Delete button's whole implementation: pick which of the two functions
 * above the selection needs, and call it. Deliberately not a third removal
 * path — the button and the Delete key end in the same two functions, so
 * what goes, where the focus lands afterwards and the undo step left behind
 * cannot drift apart between the device that has that key and the device
 * that does not.
 */
function deleteSelected() {
  if (selectedId === null) return;
  if (findToken(board, selectedId)) removeAndMoveOn(selectedId);
  else if (findArrow(board, selectedId)) removeArrowAndMoveOn(selectedId);
}

/**
 * Keeps the Delete button saying what it would remove. "Delete" on its own
 * asks a screen-reader user to work out for themselves which of a kickoff's
 * twenty-six things is selected, and the name comes from `tokenName` so the
 * button and the token it is aimed at can never call it two things.
 *
 * Disabled rather than hidden when nothing is selected: a real button's
 * disabled state is already announced and still findable, whereas a control
 * that appears and disappears is one nobody learns is there.
 */
function updateDeleteButton() {
  if (!deleteButton) return;
  const token = selectedId === null ? null : findToken(board, selectedId);
  const arrow = selectedId === null ? null : findArrow(board, selectedId);
  const what = token ? tokenName(token) : arrow ? 'the selected arrow' : null;
  deleteButton.disabled = what === null;
  deleteButton.setAttribute(
    'aria-label',
    what === null ? 'Delete — nothing is selected' : `Delete ${what}`,
  );
}
 ...
 * The one path both a straight drag and a finished bent path end at, so
 * "too short to mean anything" is judged in exactly one place for both — a
 * click-to-click path made of several tiny segments is exactly as much a
 * mis-click as a two-point drag that barely moved.
 */
...
function onArrowPointerUp(event) {
  ...
  // A drag with no path already started is the straight-arrow gesture:
  // commit the line right away rather than waiting for a click that is
  // never coming.
  if (moved && !arrowDraft) {
...
function boardUrl() {
  ...
  // A board past the codec's caps gets the bare page too, so a link that
  // would open as something other than what is on screen cannot exist —
  // not in the address bar, not on the clipboard, nowhere to be copied out
  // of by hand. `copyLink` is what says why; this is what makes sure there
  // is nothing wrong lying around for somebody to find instead.
  if (untouched || tooBigForALink(board)) return here;
  return `${here}#${SHARE_KEY}=${encode(board)}`;
}
...
async function copyLink() {
  // A board that will not fit gets a sentence instead of a link. Handing
  // over one that silently drops an arrow — or that opens as the default
  // board at the other end — is worse than not handing over anything, and
  // the message names what to take off rather than only that something is
  // wrong.
  const tooBig = tooBigForALink(board);
  if (tooBig) {
    showManualCopy(null);
    announce(tooBig);
    return;
...
function buildToolbar() {
  ...
  toolbar.innerHTML =
    ...
    `</div>` +
    // Delete has to be a control and not only a key. A phone has no Delete
    // key, so without this button a Punt preset's twenty-five tokens cannot
    // be removed at all and the only way back from one marker in the wrong
    // place is Undo walking the whole board back — the same reasoning that
    // gave Undo a button. The name it carries is written by
    // `updateDeleteButton`, since what it removes changes with the
    // selection; it starts disabled because nothing is selected yet.
    `<button type="button" class="btn btn-outline-danger btn-sm" data-delete disabled` +
    ` aria-label="Delete — nothing is selected">Delete</button>` +
  ...
  toolbar.addEventListener('click', (event) => {
    ...
    else if (target.dataset.delete !== undefined) deleteSelected();
  ...
  deleteButton = toolbar.querySelector('[data-delete]');
  ...
  hint.textContent =
    ...
    'arrow keys — hold Shift for a full yard. Whatever is selected — a marker, an arrow or a ' +
    'caption — the Delete button removes, and so does the Delete key. Ctrl/Cmd-Z undoes. ' +
    'Switch to Arrow, then drag for a straight line or click point to point and finish with ' +
    'a double-click or Enter for a bent one. Switch to Text ' +
  ...
  // Under the board, not between the toolbar and the board it describes.
  // This paragraph is the tallest block on the page at a phone's width — 378
  // of the 887 pixels that used to sit above the board on a 390x844 screen —
  // and above the board it put the field entirely below the fold, including
  // for somebody opening a shared link, which is the one case the link
  // exists for. Instructions are what you look for after seeing the thing
  // they are about, so this is also the order they read in.
  (document.querySelector('.draw-board-wrap') ?? toolbar).insertAdjacentElement('afterend', hint);
}
